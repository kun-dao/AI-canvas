...
class CapsuleNet_classifier():
    def __init__(self):
        torch.manual_seed(1)
        model = CapsNet(3)
        reconstruction_model = ReconstructionNet(16, 10)
        self.model = CapsNetWithReconstruction(model, reconstruction_model)
        self.model.load_state_dict(
            torch.load(r'CNNModel/080_model_dict_3routing_reconstructionTrue.pth', map_location=torch.device('cpu')))

    def inference (self, data):
        with  torch.no_grad():
            self.model.eval()
            output, prob = self.model.capsnet(data)
            prob, pred = prob.data.max(1, keepdim=True)  # get the index of the max probability
...
#     #     datasets.MNIST('/home/cuda90/daokun/data', train=True, download=True,
...
#     #     datasets.MNIST('/home/cuda90/daokun/data', train=False, transform=transforms.Compose([
#     #         transforms.ToTensor()
#     #     ])),
...
#     import numpy as np
#     data = Image.open('../temp-folder/number_image.png')
#     data = data.convert('L').resize((28, 28))
#     data = 255 - np.array(data)
#     Image.fromarray(data).show()
#     # data = Image.open('../minist/7.png')
#     # data.show()
#
#
...
#     print(data.shape)
#     model.load_state_dict(torch.load('./103_model_dict_3routing_reconstructionTrue.pth', map_location=torch.device('cpu')))
#     inference(data)