...
from CNNModel.capsuleModel import *
...
SELECTION_PEN = QPen(QColor(0xff, 0xff, 0xff), 1, Qt.DashLine)   #选择虚线
PREVIEW_PEN = QPen(QColor(0xff, 0xff, 0xff), 1, Qt.SolidLine)    # 画笔实线
...
class Canvas(QLabel):
    ...
    def selectpoly_mousePressEvent(self, e):
        if not self.locked or e.button == Qt.RightButton:
            ...
            self.preview_pen = SELECTION_PEN    #选择器只是把笔设置为虚线
    ...
    def fill_mousePressEvent(self, e):
        ...
        def get_cardinal_points(have_seen, center_pos):  #get the point that have seen
            points = []
            cx, cy = center_pos
    ...
    def generic_shape_timerEvent(self, final=False):
        ...
        p.setPen(pen)
        if self.last_pos:
            getattr(p, self.active_shape_fn)(QRect(self.origin_pos, self.last_pos), *self.active_shape_args)   #得到的p是标准库函数啊！！！
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for n, hex in enumerate(COLORS[0:-4:2], 1):
        ...
        self.timer.setInterval(10)     #刷新间隔：100
        self.timer.start()
        ...
        self.actionClassifier.triggered.connect(self.NumClassifier)
        ...
        self.show()
        #classifier of number
        self.classifier = CapsuleNet_classifier()
    ...
    def img_preprocessing(self, path):
        #   set proper format to model
        data = Image.open(path)
        data = data.convert('L').resize((28, 28))
    ...
    def NumClassifier(self):
        self.copy_to_clipboard()
        path = STAMPS.pop(0)
        data = self.img_preprocessing(path)
        number, prob = self.classifier.inference(data)
        print(number)
        msgBox = QMessageBox.about(self, u'Answer', u"\nNumber : {}\n Probability : {}".format(number, prob))
